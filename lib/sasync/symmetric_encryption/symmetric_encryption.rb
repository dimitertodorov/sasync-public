...
module Sasync::SymmetricEncryption
  ...
  #
  #   Sasync::SymmetricEncryption.cipher = Sasync::SymmetricEncryption::Cipher.new(
  #     :key    => '1234567890ABCDEF1234567890ABCDEF',
  #     :iv     => '1234567890ABCDEF',
  #     :cipher => 'aes-128-cbc'
  #   )
  def self.cipher=(cipher)
    raise "Cipher must be similar to Sasync::SymmetricEncryption::Ciphers" unless cipher.respond_to?(:encrypt) && cipher.respond_to?(:decrypt)
    @@cipher = cipher
  end
  ...
  def self.cipher(version = 0)
    raise "Call Sasync::SymmetricEncryption.load! or Sasync::SymmetricEncryption.cipher= prior to encrypting or decrypting data" unless @@cipher
  ...
  def self.secondary_ciphers=(secondary_ciphers)
    ...
    secondary_ciphers.each do |cipher|
      raise "secondary_ciphers can only consist of Sasync::SymmetricEncryption::Ciphers" unless cipher.respond_to?(:encrypt) && cipher.respond_to?(:decrypt)
    end
  ...
  def self.generate_symmetric_key_files(filename=nil, environment=nil)
    ...
    key_pair = Sasync::SymmetricEncryption::Cipher.random_key_pair(cipher || 'aes-256-cbc', !iv_filename.nil?)
  ...
  def self.read_config(filename=nil, environment=nil)
    ...
    if symmetric_key = (config['key'] || config['symmetric_key'])
      raise "Sasync::SymmetricEncryption Cannot hard code Production encryption keys in #{filename}" if (environment || Rails.env) == 'production'
  ...
  # Returns an instance of Sasync::SymmetricEncryption::Cipher initialized from keys
  # stored in files
  #
