module Sasync::SymmetricEncryption
  ...
  class Cipher
    ...
    #     Used by Sasync::SymmetricEncryption to select the correct key when decrypting data
    ...
    if defined?(Encoding)
      def encrypt(str)
        return if str.nil?
        buf = str.to_s.encode(Sasync::SymmetricEncryption::UTF8_ENCODING)
    ...
    if defined?(Encoding)
      def decrypt(str)
        return if str.nil?
        buf = str.to_s.force_encoding(Sasync::SymmetricEncryption::BINARY_ENCODING)
        return str if buf.empty?
        crypt(:decrypt, buf).force_encoding(Sasync::SymmetricEncryption::UTF8_ENCODING)
      end
    else
      def decrypt(str)
        return if str.nil?
